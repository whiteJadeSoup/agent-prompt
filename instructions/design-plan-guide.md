...
**3. Architecture Overview** (ASCII diagram required)
- **Draw an ASCII architecture diagram. This section is not complete without one** — prose description alone is not acceptable.
...
**4. Core Flows** (ASCII sequence or flow diagram required)
- **Draw an ASCII sequence or flow diagram for each core flow. This section is not complete without one** — skip only if the entire flow is a single linear step with no branching, error path, or concurrency.
...
4. **Diagrams over prose — default to ASCII diagrams.** Use text only to explain what isn't obvious from the diagram. Omitting a diagram requires an explicit reason (e.g., "single linear step, no branching").
